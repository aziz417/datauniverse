    ...
    <div class="form-group">
        ...
        <input id="title" type="text" value="{{ isset($weOffer) ? $weOffer->title : old('title')}}"
    ...
    <div class="form-group">
        ...
        <textarea class="form-control TextEditor" name="description"  id="description">
            {{ isset($weOffer->description) && $weOffer->description ? $weOffer->description : old('description')}}
        </textarea>
        @error('description')
        <span class="help-block m-b-none text-danger">
            {{ $message }}
    ...
    @if(isset($weOffer))
        <img class="mb-3" id="old_img" src="{{ @$weOffer->image->url }}" width="100px" height="100px" >
    @endif
    ...
    <div>
        <div class="form-group mb-0">
            <label class="mb-0">
                <input name="status"
                       {{ isset($weOffer) && $weOffer->status ? 'checked':old('status')}} type="checkbox"
                       class="i-checks">
                Publication Status
            </label>
        </div>
